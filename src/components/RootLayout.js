...
const sidebarItems = [
  [
    { id: '0', title: 'Dashboard', to: '/' },
    { id: '1', title: 'Employee', to: 'employee' },
    { id: '2', title: 'Project', to: 'project' },
    { id: '3', title: 'Asset', to: 'asset' },
  ]
...
function Sidebar({ onSidebarHide, showSidebar, handleSidebarClick }) {
  ...
  return (
    <div
      className={`fixed bg-blue-600 inset-y-0 left-0 bg-side-bar w-full sm:w-20 xl:w-60 sm:flex flex-col z-10 ${showSidebar ? 'flex' : 'hidden'
        }`}
    >
      ...
      <div className="flex-grow overflow-x-hidden overflow-y-auto flex flex-col">
        ...
        <div className="w-full mt-9 mb-6 flex items-center px-3 sm:px-0 xl:px-3 justify-start sm:justify-center xl:justify-start sm:mt-6 xl:mt-3 cursor-pointer sidebar-item"><button onClick={logout} className="w-full btn btn-error text-white btn-sm">Lougout</button></div>
      </div>
...
function MenuItem({ item: { id, title, to }, onClick, selected, handleSidebarClick }) {
  return (

    <div onClick={() => handleSidebarClick(title)}>
      <NavLink
        className={clsx(
          'w-full mt-9 flex items-center px-3 sm:px-0 xl:px-3 justify-start sm:justify-center xl:justify-start sm:mt-6 xl:mt-3 cursor-pointer',
...
function Content({ onSidebarHide, title, selectedOption }) {


  return (
    <div className="flex w-full">
      <div className="w-full fixed top-0 top-area right-0 z-9 sm:flex p-2 items-end bg-white">
        <div className="w-full hidden sm:block sm:w-20 xl:w-60 flex-shrink-0">
          .
        </div>
        <div className="sm:flex-grow h-10 flex items-center justify-between">
          <div className="">
            <div className="flex items-center">
              <div className="text-1xl font-bold text-black-500 mr-4">{selectedOption}</div>
            </div>
          </div>
          <IconButton
      ...
      <div className="content-area flex-grow overflow-x-hidden  overflow-auto flex flex-wrap content-start p-2">

        <div className="w-full p-2">
          <div className="rounded-lg bg-card sm:h-80 h-100">
            <Outlet />
          </div>
        </div>
      </div>
